  ...
  <v-app id="Home">
    ...
    <div v-if="loggedIn">
      <v-app-bar app fixed class="white bars lighten-1">
       ...
       <div v-if="txid != null">TX id : {{txid}} (for debugging check explorer)</div>
      ...
      <v-row justify="center">
        ...
        <v-btn class="mb-5 mt-5" @click="getValidatedTokenList(); Proposed = false">Get Verified Tokens</v-btn>

        <v-btn class="mb-5 mt-5" @click="getProposedTokenList(); Proposed = true">Get Proposed Tokens</v-btn>
      </v-row>
        <!--  -->
      ...
      <div v-if="Proposed == true">
        <v-container >
          ...
          <div v-for="items in propTokens" v-bind:key="items.key">
            ...
            <v-card height="400px" width="600px" class="mt-2 ml-1 rounded-t-lg ">
            <v-card-title class="white darken-2 elevation-1"><v-spacer/><v-btn>X</v-btn></v-card-title>     
              
              
                <v-col cols="12" md="12" lg="12" class="pl-5 pr-5">
                  
                  <v-row>Name :<v-spacer/>  {{items.name}}</v-row> <br>
                  <v-row> Token Address : <v-spacer/>{{items.address}}</v-row> <br>
                   <v-row>  Symbol : <v-spacer/>Symbol : {{items.symbol}} </v-row> <br>
                 <v-row>   Decimals : <v-spacer/> Decimals : {{items.decimals}} </v-row> <br>
                  <v-row>    Img : <v-spacer/> Img : {{items.image}}</v-row> <br>              
                    </v-col>
                <br>
                <v-col>
                  <v-row>

                   <v-spacer/>  upvotes : {{items.upvotes}} <v-spacer/> downvotes : {{items.downvotes}} <v-spacer/> totalvotes : {{items.totalvotes}}<v-spacer/> <br>
                  </v-row>
          ...
          <v-row justify="center" >
                  <v-btn class="green lighten-1" @click="vote(true,items.name)">Upvote</v-btn><br><v-btn @click="vote(false,items.name)" class="red">DownVote</v-btn>
                </v-row>
      ...
      <div v-if="Proposed == false">
          ...
          <div v-for="items in propTokens" v-bind:key="items.key">
            ...
            <v-card width="100%" class="pt-2 pl-2 pr-2 rounded-xl ">
            <v-card-title>{{items.name[items.count]}}</v-card-title>     
              
              
                <v-col cols="12" md="12" sm="12" class="pl-5 pr-5">
                  
                  <v-row>Name :<v-spacer/> {{items.name[items.count]}}</v-row><br>
                 <v-row> Token Address : <v-spacer/>{{items.address[items.count]}}</v-row><br>
                <v-row>  Symbol : <v-spacer/>{{items.symbol[items.count]}}</v-row> <br>
               <v-row>   Decimals : <v-spacer/>{{items.decimals[items.count]}}</v-row><br>
              <v-row>    Img : <v-spacer/>{{items.image[items.count]}}</v-row> <br>
 
                  

                </v-col>

             <img width="250px" :src="items.image[items.count]">
  ...
  methods: {
    ...
    getProposedTokenList: async function(){
     ...
     await wallet.signin();
      ...
      while (i < TokenProposals.length){
        ...
        const status = await contracts.methods
          .getStatus()
          .call();
          console.log(status)
    ...
    SubmitToken: async function(){
      ...
      await wallet.signin();
       ...
       const contract = wallet.attachToContract(unattachedContract);
       const TokenProposals = await contract.methods
         .propose_token(this.propaddr,this.porpname,this.propsymbol,this.propdec,this.propimg)
         .send(options);
         console.log(TokenProposals)
